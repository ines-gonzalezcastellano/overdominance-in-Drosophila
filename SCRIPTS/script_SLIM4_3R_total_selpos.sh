...
rm script_SLIM4_3R_total_selpos.sh.*
...
START=$(date +%s)
slim -s $num -t -m -d "Chr=\"3R\"" ./$INPUT
END=$(date +%s)
DIFF=$(( $END - $START ))
echo "Slim4 took 		$DIFF seconds" >> timefile

cp 3Rsel_list_allsnps.txt list_allsnps
sed -i -e 's/3R/1/g' list_allsnps
cp 3Rsel_dataBP.ped dataBP.ped
...
cp 3Rsel_qselpos.txt 3Rsel_qselpos$r.txt
...
cp -r 3Rsel_qselpos$r.txt $WDIR/OUTPUT_$INPUT
