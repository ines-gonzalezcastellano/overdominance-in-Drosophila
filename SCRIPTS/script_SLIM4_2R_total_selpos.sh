...
rm script_SLIM4_2R_total_selpos.sh.*
...
START=$(date +%s)
slim -s $num -t -m -d "Chr=\"2R\"" ./$INPUT
END=$(date +%s)
DIFF=$(( $END - $START ))
echo "Slim4 took 		$DIFF seconds" >> timefile

cp 2Rsel_list_allsnps.txt list_allsnps
sed -i -e 's/2R/1/g' list_allsnps
cp 2Rsel_dataBP.ped dataBP.ped
...
cp 2Rsel_qselpos.txt 2Rsel_qselpos$r.txt
...
cp -r 2Rsel_qselpos$r.txt $WDIR/OUTPUT_$INPUT
