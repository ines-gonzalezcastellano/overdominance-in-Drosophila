...
readfiles()
{
	...
	while (!feof(fdat))
	{
		...
		fscanf(fdat,"%lf", &w);
		h[s] = w;
		//if (h[s] == 1.5) printf("s=%d ps=%d ps=%llu", s, pos[s], ps[s]);
		if (ps[s] == 0.0)	h[s] = 0.0;
		for (j=1; j<=4; j++)	fscanf(fdat,"%c", &ch);
		fscanf(fdat,"%d", &x);
		fscanf(fdat,"%d", &x);
...
//		if (mut_n < 10)	printf("\n MUT_h = %f    MUT_s = %f     MUT_pos = %llu", MUT_h, MUT_s, MUT_pos);
        ...
        //only for deleterious mutations (m2), not m3 (lethals, s=-1), m4 (overdominant, s>0), and m5 (QN, h=0.5 s=-0.001)
		for (s=1; s<=numSNP; s++)
		if ((ps[s] < 0.0) && (ps[s] > (-1.0)) && (h[s] != 0.5) && (ps[s] != (-0.001)))
		{
			if (pos[s] == MUT_pos)
			{
				h[s] = MUT_h;
